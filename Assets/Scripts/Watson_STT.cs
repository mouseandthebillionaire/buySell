...
public class Watson_STT : MonoBehaviour
{
    ...
    private string _iamApikey = "mjvhwWae9nTcY-pK6Ie2cNxg5Y_8iGkMN7DfN_rlC_01";
    ...
    public string _microphoneID = "BuySell_1";
    ...
    private void OnRecognize(SpeechRecognitionEvent result, Dictionary<string, object> customData)
    {
        if (result != null && result.results.Length > 0)
        {
            foreach (var res in result.results)
            {
                ...
                {
                    string text = string.Format("{0} ({1}", alt.transcript, res.final ? "Final" : "Interim");
                    Log.Debug("ExampleStreaming.OnRecognize()", text);
                    
                    if (alt.transcript.Contains("by") && text.Contains("Final")) // needs to be final or ECHO happens
                    {
                        //Trader.S.Buy();

                    }
                    
                    if (alt.transcript.Contains("cell") ||
                        alt.transcript.Contains("shall") ||
                        alt.transcript.Contains("so")
                        && text.Contains("Final")) // needs to be final or ECHO happens
                    {
                        //Trader.S.Sell();
